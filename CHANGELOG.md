...
- 识别 Docker、Podman、LXC 等通用容器并默认隐藏或拒绝 systemd、内核、网络、Docker 和 Arch 系统级操作，避免宿主挂载或共享设施被误当作容器资源。
- 容器中的用户文件写入会检查 `HOME` 是否位于独立挂载；未经显式确认时拒绝修改可能来自宿主机的用户目录。
- Mihomo 网络二进制现在必须提供 SHA-256，配置目录、状态目录、UI 目录和 systemd unit 名均经过规范化和允许范围检查。
- 所有权标记升级为绑定规范化目标的 v2 格式；旧版无目标标记或目标重绑定时安全保留文件，不再授权删除新的路径。
- Docker 和 Mihomo 的递归清理增加允许目录与挂载边界检查；拒绝越界目标、直接挂载点、包含嵌套挂载的目录，并在无法读取挂载表时失败关闭。
- Arch Mihomo 配置测试改为通过 `-f` 读取原配置，不再覆盖并删除 `state_dir/config.yaml`。
- Ops Toolkit 不再为 `sudo`、`ssh`、`git`、包管理器和 systemd 等安全敏感命令生成 PATH 前置包装器。
