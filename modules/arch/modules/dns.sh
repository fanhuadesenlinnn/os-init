...
configure_systemd_resolved_dns() {
  local target="/etc/systemd/resolved.conf.d/90-os-init-arch-dns.conf"
...
# OS Init Arch generated. Mainland China friendly DNS baseline.
[Resolve]
DNS=${dns_servers% }
...
configure_networkmanager_dns_backend() {
  ...
  local target="/etc/NetworkManager/conf.d/90-os-init-arch-dns.conf"

  log_info "配置 NetworkManager 使用 systemd-resolved：${target}"

  write_root_file_from_stdin "${target}" 0644 <<'EOF'
# OS Init Arch generated. Keep NetworkManager and systemd-resolved aligned.
[main]
