...
doctor_check_config_file() {
  local config_file
  config_file="${OS_INIT_ARCH_CONFIG_FILE:-${HOME}/.config/os-init/config.env}"
  if [[ "${OS_INIT_ARCH_LOAD_CONFIG_FILE:-1}" -ne 1 ]]; then
...
show_doctor() {
  ...
  if [[ "${EUID}" -eq 0 ]]; then
    doctor_check "privilege" "ok" "root 模式不需要 sudo"
  else
