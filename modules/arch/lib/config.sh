...
config_scalar_keys() {
  cat <<'EOF'
ASSUME_YES DRY_RUN OS_INIT_ARCH_USE_STATE OS_INIT_ARCH_STATE_DIR OS_INIT_ARCH_LOAD_CONFIG_FILE OS_INIT_ARCH_CONFIG_FILE OS_INIT_ARCH_JSON_SCHEMA_VERSION PACMAN_RETRY_ATTEMPTS ARCHLINUXARM_MIRRORS
ENABLE_CHINA_MIRROR GITHUB_PROXY
...
config_bool_keys() {
  cat <<'EOF'
ASSUME_YES DRY_RUN OS_INIT_ARCH_USE_STATE OS_INIT_ARCH_LOAD_CONFIG_FILE ENABLE_CHINA_MIRROR
...
VMWARE_FORCE_SOFTWARE_RENDERER VM_HYPRLAND_DYNAMIC_RESIZE VM_HYPRLAND_LOW_LATENCY INSTALL_HYPRDOTS_OBSIDIAN MIHOMO_AUTO_ENABLE_SERVICE MIHOMO_ALLOW_LAN ENABLE_METACUBEXD
EOF
}
...
apply_config_assignment() {
  local key="$1" value="$2"
  # Migrate the former Arch-only name without exporting two settings for the
  # same behavior.
  [[ "${key}" == "PROXY_AUTO_ENABLE_SERVICE" ]] && key="MIHOMO_AUTO_ENABLE_SERVICE"
