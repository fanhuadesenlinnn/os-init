...
- `Arch 最小基础`：只从官方仓库安装下载、解压和基础诊断工具，不强制接管第三方软件源。
- `Arch 现代 CLI`：通过 pacman/archlinuxcn 安装 ripgrep、fzf、bat、eza、dust、bottom、procs 等工具；Git、tmux、编译器仍由各自模块管理。
- `AUR Helper`：root 和普通用户均优先从 archlinuxcn 用 pacman 安装 paru；已有 yay 仅作为兼容回退保留，OS Init 不再主动安装两个 helper；仅普通用户可回退到 makepkg。
...
- `Git / GitHub`：使用跨平台稳定模块 ID `git`；在 Arch 由原生实现安装 git、gh、OpenSSH，并写入目标用户 Git 配置。
...
- `Mihomo + MetaCubeXD`：使用跨 Linux 稳定模块 ID `mihomo`；在 Arch 从 pacman/archlinuxcn 安装，写入 `/etc/mihomo`、在 `/var/lib/mihomo` 按发行版 systemd unit 运行方式预检配置，并部署 MetaCubeXD。
